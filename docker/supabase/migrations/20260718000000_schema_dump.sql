...
CREATE FUNCTION public.bind_embedded_machine(embedded_id_ uuid, machine_id_ uuid) RETURNS void
    LANGUAGE plpgsql
    AS $$
begin
...
CREATE FUNCTION public.debit_product_stock_on_coil_refill() RETURNS trigger
    LANGUAGE plpgsql
    AS $$                                                            
  DECLARE
      v_diff integer;                                                                               
...
CREATE FUNCTION public.fill_sale_coil_data() RETURNS trigger
    LANGUAGE plpgsql
    AS $$BEGIN
...
CREATE FUNCTION public.get_machine_name(machine_id uuid) RETURNS text
...
--
-- Name: register_embedded(text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.register_embedded(mac_address_ text) RETURNS TABLE(id uuid, subdomain bigint, passkey text, status public.embedded_status)
    LANGUAGE plpgsql
    AS $$
  BEGIN
    RETURN QUERY
      SELECT e.id, e.subdomain, e.passkey, e.status
      FROM public.embedded e
      WHERE e.mac_address = mac_address_ AND e.owner_id = auth.uid()
      LIMIT 1;
    ...
    RETURN QUERY
      INSERT INTO public.embedded (mac_address)
      VALUES (mac_address_)
      RETURNING embedded.id, embedded.subdomain, embedded.passkey, embedded.status;
  END;
  $$;
...
CREATE FUNCTION public.sync_machine_coils_on_model_change() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE TABLE public.machine_coils (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    product_id uuid,
...
CREATE TABLE public.model_coils (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    alias text,
    item_number smallint,
...
CREATE VIEW public.sales_metrics_v1 WITH (security_invoker='on') AS
 SELECT s.machine_id,
...
--
-- Name: metrics select_policy; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY select_policy ON public.metrics FOR SELECT USING (((deleted_at IS NULL) AND (EXISTS ( SELECT 1
   FROM public.embedded e
...
--
-- Name: metrics update_policy; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY update_policy ON public.metrics TO authenticated USING ((EXISTS ( SELECT 1
   FROM public.embedded e
  WHERE ((e.id = metrics.embedded_id) AND (e.owner_id = auth.uid()))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM public.embedded e
  WHERE ((e.id = metrics.embedded_id) AND (e.owner_id = auth.uid())))));
