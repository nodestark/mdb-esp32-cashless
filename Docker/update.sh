...
# ─── MQTT ACL credentials (required for broker auth) ─────────────────────────
if [ -z "${MQTT_FORWARDER_PASS:-}" ] || [ -z "${MQTT_BACKEND_PASS:-}" ]; then
    info "MQTT credentials not found — generating for broker ACL..."
...
MQTTEOF
    ...
    success "MQTT credentials generated, passwd file updated, and appended to .env"
else
    success "MQTT credentials present"
fi
...
docker compose up -d --no-deps frontend broker forwarder
